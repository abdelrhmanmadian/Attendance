...
import { buildDailyAttendanceReport } from "../services/reportService.js";
...
reportsRouter.get("/attendance", async (req, res) => {
  ...
  const rows = await buildDailyAttendanceReport(parsed.data.from, parsed.data.to, parsed.data.doctorId);
  res.json(rows);
});
